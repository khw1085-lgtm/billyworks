...
export function SearchOverlay({ apartments, onClose, onSelect }: Props) {
  ...
  return <div className="search-overlay" role="dialog" aria-modal="true" aria-label="아파트 검색">
    ...
    <div className="search-results"><p>{query ? `검색 결과 ${results.length}개` : '많이 찾는 아파트'}</p>{results.map((apartment) => <button key={apartment.id} onClick={() => onSelect(apartment)}><span><strong>{apartment.name}</strong><small>{apartment.address}</small></span><b><i>장점 {apartment.positiveCount ?? 0}</i><em>불만 {apartment.negativeCount ?? 0}</em></b></button>)}</div>
  </div>
}
