...
export function BoardScreen(props: Props) {
  ...
  return <main className="board-page" id="main-content">
    ...
    <div className="board-controls"><strong>익명 한마디 <b>{visible.length}</b><span className="board-sentiment-count positive">장점 {props.apartment.positiveCount ?? 0}</span><span className="board-sentiment-count negative">불만 {props.apartment.negativeCount ?? 0}</span></strong><label>정렬<select value={props.sort} onChange={(event) => props.onSort(event.target.value as Sort)}><option value="latest">최신순</option><option value="agree">공감순</option><option value="comments">댓글순</option></select><ChevronDown size={15} /></label></div>
    ...
    <div className="all-topics" aria-label="제공되는 장단점 주제">{categories.map((category) => <span key={category}>{category}</span>)}</div>
  </main>
}
