...
type Props = { apartments: Apartment[]; posts: Post[]; selected?: Apartment; onSelect: (item?: Apartment) => void; onWrite: (item: Apartment) => void }

export function MapHome({ apartments, posts, selected, onSelect, onWrite }: Props) {
  ...
  return <main className="map-home" id="main-content">
    ...
    <MapCanvas apartments={apartments} selectedId={selected?.id} zoom={zoom} onZoom={setZoom} onSelect={(item) => onSelect(item)} />
