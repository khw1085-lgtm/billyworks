...
export function HotScreen(props: Props) {
  ...
  return <main className="hot-page" id="main-content">
    <header className="section-header"><button onClick={() => navigate('/')} aria-label="뒤로"><ArrowLeft size={21} /></button><span><Flame size={18} />전국에서 반응이 커요</span><h1>인기 불만</h1><p>거친 말보다 많은 사람이 공감한 생활 문제를 먼저 보여드려요.</p></header>
    ...
    <div className="hot-apartment-links">{props.apartments.filter((item) => item.trending).map((item) => <button key={item.id} onClick={() => navigate(apartmentPath(item.id))}><Flame size={16} /><span><strong>{item.name}</strong><small>{item.region} · 불만 {item.postCount}개</small></span></button>)}</div>
